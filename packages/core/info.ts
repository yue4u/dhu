...
export type GetInfoOptions = {
  ...
  skipRead?: boolean;
  content?: boolean;
  attachmentsOptions?: HandleAttachmentOptions;
  sync?: boolean;
};
...
export async function getInfoItemByIndex(
  ...
  let ret: Info = { title };
  if (!(options.content || options.attachmentsOptions)) {
    return ret;
  }
  await page.evaluate(
    ([selector, i]) => {
  ...
  if (options.attachmentsOptions) {
    ...
    if (hasAttachments) {
      ...
      attachments = await handleDownloadTable(page, options.attachmentsOptions);
    }
    ret = { ...ret, attachments };
  }
